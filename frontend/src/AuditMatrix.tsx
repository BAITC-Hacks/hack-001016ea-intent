...
const authorityNames: Record<string,string> = {duty:"обязанность",right:"право",prohibition:"запрет",unknown:"полномочие не определено"};
...
export function AuditMatrix({record,openFinding,openSources}: {record:Audit; openFinding:(f:Finding)=>void; openSources:(ids:string[])=>void}) {
  ...
  const searches = useMemo(()=>Object.fromEntries(record.search_results.map(s=>[s.id,s])),[record]);
  ...
  return <section className="panel matrix-panel">
    ...
    <div className="matrix-scroll"><table className="audit-matrix"><thead><tr><th>Функция и владелец до</th><th>Функции и владельцы после / кандидаты</th><th>Результат</th><th>Доказательства</th></tr></thead><tbody>{rows.slice(current*20,(current+1)*20).map(row=>{
      ...
      return <tr key={row.id}><td>{before ? <><strong>{units[before.unit_id]}</strong><p>{before.text}</p>{before.context_text && <details><summary>Контекст и полномочие</summary>{authorityNames[before.authority]} · {before.context_text}</details>}</> : <p>Прежнее соответствие не установлено. Доступен поиск по комплекту «до».</p>}</td><td>{row.after_claim_ids.length ? row.after_claim_ids.map(id=><div className="matrix-claim" key={id}><strong>{units[claims[id].unit_id]}</strong><details><summary>{claims[id].text.slice(0,160)}{claims[id].text.length>160?"…":""}</summary><p>{claims[id].text}</p><small>{authorityNames[claims[id].authority]} · {claims[id].context_text||"Отдельный контекст не указан"}</small></details></div>):<p>Явный владелец не подтверждён. Проверить журнал поиска.</p>}</td><td><span className={`badge ${row.status==="PRESERVED"?"green":row.status==="MOVED"?"blue":"amber"}`}>{names[row.status]||row.status}</span>{row.finding_ids.map(id=>{const f=record.findings.find(f=>f.id===id); return f ? <button className="text-button" key={id} onClick={()=>openFinding(f)}>{f.title}</button>:null})}</td><td><button className="text-button" onClick={()=>openSources(row.source_span_ids)}>Открыть {row.source_span_ids.length} фрагм.</button>{row.source_span_ids.slice(0,3).map(id=><small className="matrix-locator" key={id}>{spans[id].version==="before"?"До":"После"} · {documents[spans[id].document_id].name} · §{spans[id].clause||"—"}</small>)}{row.search_result_ids.map(id=>{const search=searches[id];return search ? <details key={id}><summary>Поиск по комплекту «{search.searched_version==="before"?"до":"после"}»</summary><p>Запрос: {search.query}</p><p>{search.searched_document_ids?.length||1} файл(ов) · {search.searched_span_ids.length} фрагм. · {search.exact_match_count} точных совпадений</p><p>{search.limitation}</p>{!!search.matched_span_ids?.length && <button className="text-button" onClick={()=>openSources(search.matched_span_ids!)}>Открыть найденные фрагменты</button>}</details>:null})}</td></tr>
    })}</tbody></table></div>
