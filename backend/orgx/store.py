...
class Store:
    ...
    def get(self, key):
        with self.connect() as db:
            row = db.execute(
                "SELECT body FROM artifacts WHERE key=?", (key,)
            ).fetchone()
    ...
    def put(self, key, value):
        with self.connect() as db:
            db.execute(
                "INSERT OR IGNORE INTO artifacts VALUES (?,?)", (key, canonical(value))
            )
        return self.get(key)
    ...
    def review(self, audit_id, finding_id, status, actor, note):
        with self.connect() as db:
            db.execute(
                "INSERT INTO reviews (audit_id,finding_id,status,actor,note,created_at) VALUES (?,?,?,?,?,?)",
                (
                    audit_id,
                    finding_id,
                    status,
    ...
    def reviews(self, audit_id):
        with self.connect() as db:
            ...
            return [
                dict(r)
                for r in db.execute(
                    "SELECT * FROM reviews WHERE audit_id=? ORDER BY id", (audit_id,)
                )
            ]
