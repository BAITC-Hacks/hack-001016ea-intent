...
TRUTH=json.loads((Path(__file__).resolve().parents[2]/'truth/source_truth.json').read_text())
...
def test_every_truth_quote_is_exact_and_source_hash_matches(real_documents):
    ...
    for case in TRUTH['cases']:
        ...
        for ev in case['evidence']:
            span=next(s for s in docs[ev['version']].spans if s.paragraph_id==ev['paragraph_id'])
...
def test_hand_reviewed_expectations(real_audit):
    for case in TRUTH['cases']:
        e=case['expected']
        if 'unit_status' in e:
            assert any(c.status==e['unit_status'] and e['unit_contains'] in c.title for c in real_audit.unit_changes),case['id']
        if 'finding_type' in e:
            matching=[f for f in real_audit.findings if f.type==e['finding_type']]
            if 'before_paragraph' in e:
                matching=[f for f in matching if f.before_claim_id and f.before_claim_id.endswith(e['before_paragraph']+':claim')]
            if 'rule_id' in e:
...
def test_unsupported_cases_are_actually_blocked(real_audit):
    codes={c['expected']['forbidden_code'] for c in TRUTH['cases'] if c['category']=='unsupported'}
    assert len(codes)==7
    ...
    assert any(f.type=='PRESERVED' and f.before_claim_id.endswith('p0186:claim') for f in real_audit.findings if f.before_claim_id)
    moved=[f for f in real_audit.findings if f.type=='MOVED']
    units={u.id:u for u in real_audit.ir.units};claims={c.id:c for c in real_audit.ir.claims}
    assert all(units[claims[c].unit_id].kind=='collective' for f in moved for c in f.after_claim_ids)
    conflict=next(f for f in real_audit.findings if f.rule_id=='governance-independence-tension-v1')
    assert conflict.type=='POTENTIAL_AUTHORITY_CONFLICT'
    assert conflict.evidence_against and conflict.review_status=='PENDING'
    empty=[f for f in real_audit.findings if f.before_claim_id and f.before_claim_id.endswith('p0187:claim')]
...
def test_matching_batches_search_whole_after_version(real_audit):
    ids={s.id for s in real_audit.ir.documents[1].spans}
...
def test_company_resource_duties_are_not_attributed_to_auditors(real_audit):
    ...
    c=next(c for c in real_audit.ir.claims if c.version=='after' and c.source_span_ids[0].endswith(':p0250'))
