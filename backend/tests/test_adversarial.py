    ...
    for version, items in [("before", before), ("after", after)]:
        ...
        for i, item in enumerate(items):
            ...
            units.append(
                Unit(
                    ...
                    kind="role",
                    source_span_ids=[sid],
                    department_ids=[f"{version}:department:{owner}"],
                )
            )
            claims.append(
                FunctionClaim(
        ...
        docs.append(
            Document(
                ...
                version=version,
                name=version + ".docx",
                sha256=digest(str(items).encode()),
                format="docx",
                spans=spans,
            )
        )
...
@pytest.mark.parametrize(
    ...
    [
        (
            [("Проверяет сохранность активов компании", "A")],
            [("Обеспечивает сохранность активов компании", "A")],
        ),
        (
            [("Контролирует устранение недостатков", "A")],
            [("Организует мониторинг корректирующих мер", "A")],
        ),
        (
            [("Проверяет все риски компании", "A")],
            [("Проверяет финансовые риски", "B"), ("Проверяет ИТ-риски", "C")],
        ),
        (
            [("Контролирует устранение нарушений", "A")],
            [("Контролирует качество устранения нарушений", "A")],
        ),
        (
            [("Формирует группы контроля качества", "A")],
            [("Ведёт переписку по кадровым вопросам", "B")],
        ),
        (
            [("Проверяет все риски компании", "A")],
            [
                ("Проверяет все риски компании", "B"),
                ("Проверяет все риски компании", "C"),
            ],
        ),
        (
            [("Утверждает план аудита", "A", "всех подразделений")],
            [("Утверждает план аудита", "A", "ИТ-департамента")],
        ),
        (
            [("Принимает управленческие решения", "A", "", "right")],
            [("Принимает управленческие решения", "A", "", "prohibition")],
        ),
        (
            [("Проверяет качество финансовой отчетности", "A")],
            [("Не проверяет качество финансовой отчетности", "A")],
        ),
    ],
)
def test_unsupported_certainty_never_replaces_review(before, after):
    ...
    assert all(
        f.type not in ("PRESERVED", "MOVED", "POTENTIAL_GAP") for f in result.findings
    )
...
def test_exact_context_preserved():
    r = scenario(
        [("Проверяет качество отчетности", "A")],
        [("Проверяет качество отчетности", "A")],
    )
...
def test_determinism():
    args = (
        [("Проверяет качество отчетности", "A")],
        [("Оценивает качество отчетности", "A")],
    )
