...
with tempfile.TemporaryDirectory() as directory:
    ...
    for mode in ['cold','warm']:
        start=perf_counter();response=client.post('/api/demo');seconds=perf_counter()-start
        response.raise_for_status();payload=response.json();outputs.append(canonical(payload['record']))
